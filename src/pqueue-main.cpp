...
string randomNumberString(int length = 8) {
    string result;
    for (int i = 0; i < length; i++) {
        result += char(randomInteger('1', '9'));
    }
    return result;
}
...
template <typename PQueue>
    ...
    try {
         ...
         */
        {
            logInfo("These tests will check size() isEmpty() without calling dequeue().");
         ...
         */
        {
            logInfo("We're about to start calling dequeue().");
            PQueue queue;
        ...
        {
            logInfo("This next test will check whether peek() matches dequeue().");
            PQueue queue;

            for (int i = 0; i < 5; i++) {
                string s = randomNumberString();
                queue.enqueue(s, stringToInteger(s));
            }

            while (!queue.isEmpty()) {
                string expected = queue.peek();
                checkCondition(queue.dequeue() == expected, "Value returned by peek() matches value returned by dequeue()");
            }
        }

        /* Getting harder: The priority dequeued should always match the value of peekPriority(). */
        {
            logInfo("This next test will check whether peek() matches dequeue().");
            PQueue queue;
            ...
            while (!queue.isEmpty()) {
                int expected = queue.peekPriority();
                checkCondition(stringToInteger(queue.dequeue()) == expected, "Value returned by peek() matches value returned by dequeue()");
            }
        }

        /* Clearing the queue should remove elements. */
        {
            logInfo("This next test will check whether clear() removes all elements.");
            PQueue queue;
            ...
            queue.clear();
            checkCondition(queue.isEmpty(), "After calling clear(), the queue should be empty.");
        }
        ...
        /* In the same vein - what happens if we dequeue from an empty queue? */
        {
            ...
            checkCondition(didThrow, "Priority queue uses 'error' when dequeue() called on empty queue.");
        }

        /* Test peekPriority on empty queue.
         */
        {
            PQueue queue;
            bool didThrow = false;
            try {
                logInfo("About to peekPriority into an empty queue.  This may cause a");
                logInfo("crash if your implementation is incorrect.");
                queue.peekPriority();
            } catch (ErrorException&) {
                didThrow = true;
            }

            checkCondition(didThrow, "Priority queue uses 'error' when peekPriority() called on empty queue.");
        }
 ...
 * in either your enqueue() or dequeue() functions.
 */
template <typename PQueue>
    ...
    try {
         ...
         */
        {
            ...
            for (char ch = 'A'; ch <= 'H'; ch++) {
                 ...
                 */
                queue.enqueue(string(1, ch), ch);
            }

            for (char ch = 'A'; ch <= 'H'; ch++) {
                string expected(1, ch);
                checkCondition(queue.dequeue() == expected, "Queue should yield " + expected + ".");
            }
        }
         ...
         */
        {
            ...
            for (int i = 0; i < 10; i++) {
                randomValues += randomNumberString();
            }
            ...
            for (string value : randomValues) {
                queue.enqueue(value, stringToInteger(value));
            }

            /* Confirm each comes back correctly. */
            for (int i = 0; i < randomValues.size(); i++) {
                checkCondition(queue.dequeue() == randomValues[i],
         ...
         */
        {
            ...
            bool isCorrect = true;
            for (int i = 0; i < randomValues.size(); i++) {
                if (queue.dequeue() != randomValues[i]) {
                    isCorrect = false;
                    break;
                }
            }
    ...
    try {
         ...
         */
        {
            ...
            for (string letter : letters)
                queue.enqueue(letter, letter[0] - 'A');
    ...
    try {
         ...
         */
        {
            ...
            for (char ch = 'A'; ch <= 'D'; ch++) {
                queue.enqueue(string(1, ch), ch - 'A');
                queue.enqueue(string(1, ch), ch - 'A');
            }
...
template <typename PQueue>
    ...
    try {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            for (int i = 0; i < 10000; i++) {
                strings += randomNumberString();
            }

            /* Enqueue them into the priority queue. */
            logInfo("Enqueuing 10000 random strings.");
            for (string str : strings) {
                queue.enqueue(str, stringToInteger(str));
            }
...
void printReplInstructions() {
    ...
    cout << endl;
    cout << "   [i]sEmpty:            Reports whether the priority queue is empty." << endl;
    cout << "   [s]ize:               Reports the size of the priority queue" << endl;
    cout << "   [e]nqueue <str> <p>:  Enqueues the string <str> with priority <p>" << endl;
    cout << "   [p]eek:        Peeks at the minimum element of the priority queue." << endl;
    cout << "   [d]equeue:     Dequeues the minimum element of the priority queue." << endl;
    cout << "   [q]uit:               Quits the interpret and returns to the menu." << endl;
    cout << endl;
            ...
            if (!command) {
                ...
                try {
                    if (action == "peek" || action == "p") {
                        ...
                        string toEnqueue;
                        int priority;
                        if (!(command >> toEnqueue >> priority)) {
                            cout << "Must specify a string to enqueue and a priority." << endl;
                        } else {
                            queue.enqueue(toEnqueue, priority);
                            cout << "Enqueued string \"" << toEnqueue << "\". with priority " << priority << "." << endl;
                        }
                    } else {
 ...
 */
void displayMenu() {
    cout << "CS106X Priority Queue Testing Harness" << endl;
    cout << "=====================================" << endl;
    cout << REPL_VECTOR << ": Manually test VectorPriorityQueue" << endl;
    cout << TEST_VECTOR << ": Automatically test VectorPriorityQueue" << endl;
    cout << REPL_DOUBLY_LINKED_LIST << ": Manually test LinkedPriorityQueue" << endl;
    cout << TEST_DOUBLY_LINKED_LIST << ": Automatically test LinkedPriorityQueue" << endl;
    cout << REPL_HEAP << ": Manually test HeapPriorityQueue" << endl;
    cout << TEST_HEAP << ": Automatically test HeapPriorityQueue" << endl;
    cout << REPL_BINOMIALHEAP << ": Manually test BinomialHeapPriorityQueue" << endl;
    cout << TEST_BINOMIALHEAP << ": Automatically test BinomialHeapPriorityQueue" << endl;
    cout << QUIT << ": Quit" << endl;
}
...
int main() {
    while (true) {
        ...
        case TEST_DOUBLY_LINKED_LIST:
            testPriorityQueue<LinkedPriorityQueue> ();
            break;
        case REPL_DOUBLY_LINKED_LIST:
            replTestPriorityQueue<LinkedPriorityQueue> ();
            break;
        case TEST_HEAP:
        ...
        case REPL_HEAP:
            ...
            break;
        case TEST_BINOMIALHEAP:
            testPriorityQueue<BinomialHeapPriorityQueue> ();
            break;
        case REPL_BINOMIALHEAP:
            replTestPriorityQueue<BinomialHeapPriorityQueue> ();
            break;
        case QUIT:
            return 0;
        default:
